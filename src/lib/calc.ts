import type { Transaction, Asset, Holding, AssetType, Currency } from '../types'
...
export function computeQuarterlyPortfolioGrowth(
  ...
  for (let cursor = firstQuarterStart; cursor <= currentQuarterStart; cursor = addQuarter(cursor)) {
    ...
    while (transactionIndex < sorted.length) {
      ...
      if (tx.action === 'dividend') { transactionIndex += 1; continue }
      if (cashLedger && tx.action !== 'deposit' && tx.action !== 'withdraw') {
        transactionIndex += 1
        continue
      }
